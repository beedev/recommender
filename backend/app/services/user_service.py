...
Bharath's Quality-First Implementation providing user CRUD operations,
...
class UserService:
    ...
    async def change_password(self, session: AsyncSession, user_id: str, 
        ...
        await self.auth_service.revoke_all_user_tokens(str(user.id), session)
    ...
    async def deactivate_user(self, session: AsyncSession, user_id: str, 
        ...
        await self.auth_service.revoke_all_user_tokens(user_id, session)
        await session.commit()
